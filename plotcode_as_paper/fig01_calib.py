...
for p, contrib in enumerate(contrib_ids):
    ...
    cols = [cm.Accent_r(np.float(k) / len(observations))
    ...
    for i, obs in enumerate(observations):
        ...
        sl_calculated = np.zeros(
            [gmt_anom.shape[0], len(params.commitment_parameter)])
        for j, alpha in enumerate(params.commitment_parameter):
            ...
            sl_calculated[:, j] = sl_contributor.calc_contribution(
                gmt_anom, tau)
