...
nat_model_mean = gcd.marzeion_gic_nat_up.mean(axis="model") / 1000.0  # in m
natural_gic_offset = nat_model_mean[2012] - nat_model_mean[1851]
gic_equi_marzeion12 = gic_equ_marzeion12["sle_equi"] / 1000.0 - natural_gic_offset
...
# preindustrial
gic_temp_radic = np.arange(0.0, 7.0, 1.0)
...
def get_equi_coefficients(temp, equi_estimate):
    ...
    popt, pcov = optimize.curve_fit(
        gic_equi_func, temp_nonan, equi_estimate_nonan, p0=[1.0, 1.0]
    )
    return popt
...
np.savetxt(
    os.path.join(basedir, "data/glacier_equi/glacier_equi_coefficients.csv"),
    gic_equi_coeffs,
    fmt="%.8f",
    header="a        b",
)
