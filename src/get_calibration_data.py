...
sealevelbudget = np.loadtxt(inputdatadir+"Observations/SeaLevelBudgetChurch.txt")#,delimiter=" ")
budgetlbl = ["satellite","tide gauge","dummy","thermexp","gic","gis","ant","","terrestrial_storage"]
...
## Survey in Geophysics 2011, link.springer.com/article/10.1007%2Fs10712-011-9119-1
church_past_data  = np.loadtxt(inputdatadir+"church_white11/CSIRO_Recons_gmsl_yr_2011.txt")
...
hay15_past_sl  = da.DimArray(fn(np.arange(1901,2011,1)),axes=np.arange(1901,2011,1),dims="time")

haydata = np.loadtxt(inputdatadir+"hay2015_sealevel/hay15_lower.csv",delimiter=",")
fn = scipy.interpolate.interp1d(haydata[:,0],  haydata[:,1]*1e-3) # in mm
hay15_lower  = da.DimArray(fn(np.arange(1901,2010,1)),axes=np.arange(1901,2010,1),dims="time")

haydata = np.loadtxt(inputdatadir+"hay2015_sealevel/hay15_upper.csv",delimiter=",")
fn = scipy.interpolate.interp1d(haydata[:,0],  haydata[:,1]*1e-3) # in mm
hay15_upper  = da.DimArray(fn(np.arange(1901,2010,1)),axes=np.arange(1901,2010,1),dims="time")
...
thermo_obs_domingues08 = da.DimArray(domingues08[:,1],axes=domingues08[:,0]-0.5,dims="time")/1000.
...
ishii09 = np.loadtxt(inputdatadir+"thermosteric_ishii2009/global_mean_thermosteric_0-700m.txt",skiprows=2)
thermo_obs_ishii09 = da.DimArray(ishii09[:,1],axes=ishii09[:,0],dims="time")/1000.
...
levitus12_700m  = np.loadtxt(inputdatadir+"thermosteric_levitus2012/pent_a-mm-w0-700m.dat",skiprows=1)
levitus12_2000m = np.loadtxt(inputdatadir+"thermosteric_levitus2012/pent_a-mm-w0-2000m.dat",skiprows=1)
thermo_obs_levit12_700 = da.DimArray(levitus12_700m[:,1],axes=levitus12_700m[:,0]-0.5,dims="time")/1000.
thermo_obs_levit12_2000 = da.DimArray(levitus12_2000m[:,1],axes=levitus12_2000m[:,0]-0.5,dims="time")/1000.

## Purkey & Johnson 2010
## http://journals.ametsoc.org/doi/abs/10.1175/2010JCLI3682.1
## estimates for deep ocean below 2000m
## overlong time for trend, the length will be shortened by upper ocean observation length.
purkey10_below2000m = da.DimArray(np.arange(64)*0.113e-3,axes=np.arange(1951,2015,1),dims="time")
purkey10_below3000m = da.DimArray(np.arange(64)*0.097e-3,axes=np.arange(1951,2015,1),dims="time")

## Llovel et al 2014
## www.nature.com/articles/nclimate2387
llovel14_below2000m = da.DimArray(np.arange(len(years_church))*-0.13e-3,axes=years_church,dims="time")

## trend 700-3000m, see paper
## time axis is artifically expanded here. will be restricted by upper ocean observations.
church_observed["te_700_3000"]   = da.DimArray(np.arange(64)*0.07e-3,axes=np.arange(1951,2015,1),dims="time")
## trend below 3000
church_observed["te_below_3000"] = da.DimArray(np.arange(64)*0.09e-3,axes=np.arange(1951,2015,1),dims="time")

church_observed_700m = church_observed["thermexp"] - church_observed["te_700_3000"] - church_observed["te_below_3000"]
church_observed_700m_2000m = church_observed["te_700_3000"] + church_observed["te_below_3000"] - purkey10_below2000m



#### thermal expansion from cmip5 models, taken from fig 13.4 ch13 ipcc ar5 wg1
ipcc_te_data  = np.loadtxt(inputdatadir+"ipcc_ar5/thermal_contribution_ch13_fig13p4.csv")
f = scipy.interpolate.interp1d(ipcc_te_data[:,0],  ipcc_te_data[:,1]*1e-3) # in mm
ipcc_thermal_expansion = da.DimArray(f(np.arange(1902,2010,1)),axes=np.arange(1902,2010,1),dims="time")
ipcc_thermal_expansion -= ipcc_thermal_expansion[1986:2005].mean()

# thermal expansion range 1901-1990, table 13.1 ch 13 IPCC
# in mm
ipcc_te_lower = da.DimArray(np.linspace(0.,0.06,1991-1901)*(1991-1901),axes=np.arange(1901,1991,1),dims="time")
ipcc_te_med   = da.DimArray(np.linspace(0.,0.37,1991-1901)*(1991-1901),axes=np.arange(1901,1991,1),dims="time")
ipcc_te_upper = da.DimArray(np.linspace(0.,0.67,1991-1901)*(1991-1901),axes=np.arange(1901,1991,1),dims="time")
...
gls = scipy.io.loadmat(inputdatadir+"glaciers_marzeion/glacier_mass_change_for_matthias.mat")
...
fullfile = inputdatadir+"glaciers_marzeion14/global_cumulative_mass_loss_full_rgi_v4.txt"
...
anthropogenic_frac_perc = da.from_pandas(anthropogenic_frac_perc,dims=["percentile","time"])
...
anthropogenic_frac_lin_1900 = da.DimArray( 6.78606933e-03*tlin-1.28443788e+01, axes=tlin,dims="time")
...
def convert_raw_mb_to_sl(rawdata):
  ...
  # in Gt/y
  mb = da.DimArray(fn(np.arange(1871,2009,1)),axes=np.arange(1871,2009,1),dims="time")
  ...
  # assume "equilibrium" before 1900, see the zero before 1900 in Fig. 7, Box and Colgan 2013
  mb -= mb[1870:1900].mean()
  # convert to m SLE
  return mb.cumsum()/360./1.e3

# Box and Colgan, Journal of Climate 2013, reconstruction of total Greenland mass balance
boxdata = np.loadtxt(inputdatadir+"greenland_box/box_colgan2013_fig6.csv",delimiter=",")
boxdata[:,1] *= -1
...
fn = scipy.interpolate.interp1d(boxdata[:,0],boxdata[:,1]/360./1.e3) # in m/y SLE
box_smb_rate = da.DimArray(fn(np.arange(1871,2009,1)),axes=np.arange(1871,2009,1),dims="time")
...
rawdata = np.loadtxt(inputdatadir+"greenland_sasgen12/sasgen_vandenbroeke12_smb.csv",delimiter=",")
fn = scipy.interpolate.interp1d(rawdata[:,0],-1.*rawdata[:,1]/360./1.e3) # in m SLE
sasgen_smb = da.DimArray(fn(np.arange(1959,2009,1)),axes=np.arange(1959,2009,1),dims="time")
...
rawdata = np.loadtxt(inputdatadir+"greenland_vanangelen14/vanangelen_vandenbroeke14_fig11.csv",
    delimiter=",")
fn = scipy.interpolate.interp1d(rawdata[:,0],-1.*rawdata[:,1]/360.) # in m SLE
angelen_smb_since_1992 = da.DimArray(fn(np.arange(1990,2013,1)),axes=np.arange(1990,2013,1),dims="time")
rawdata = np.loadtxt(inputdatadir+"greenland_vanangelen14/vanangelen_vandenbroeke14_fig10.csv",
    delimiter=",")
fn = scipy.interpolate.interp1d(rawdata[:,0],-1.*rawdata[:,1]/360./1000.) # in m SLE
...
rawdata = np.loadtxt(inputdatadir+"greenland_sasgen12/sasgen_vandenbroeke12_sid.csv",delimiter=",")
fn = scipy.interpolate.interp1d(rawdata[:,0],-1.*rawdata[:,1]/360./1.e3) # in m SLE
sasgen_sid = da.DimArray(fn(np.arange(1959,2009,1)),axes=np.arange(1959,2009,1),dims="time")

#### andresen straneo nature geoscience 2012, 1.5 degC (not used currently)
gis_ocean_warming_obs_1992_2008 = da.DimArray(np.linspace(0,1.5,2009-1992),axes=np.arange(1992,2009,1),dims="time")
...
mouginot_ase_sl = da.DimArray(np.cumsum(contrib_to_slr),axes=np.arange(1974,2014),dims="time")*gt_to_m
# church_observed["Antarctica SID"] = mouginot_ase_sl#church_observed["Antarctica"]

## Harig and Simons 2015, total antarctic mass change in Gt/y
harig_data = np.loadtxt(inputdatadir+"harig_simons15/harig_simons15_fig2e_sm.csv",delimiter=",")
f = scipy.interpolate.interp1d(harig_data[:,0],harig_data[:,1])
harig_simons15 = da.DimArray(f(np.arange(2003,2014)),axes=np.arange(2003,2014),dims="time")*-1*gt_to_m
