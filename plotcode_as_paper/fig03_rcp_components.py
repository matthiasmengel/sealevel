...
for i, name in enumerate(contrib_ids[0:6]):
    ...
    axy = divider.append_axes(
        "right", size=0.6, pad=0.0, sharey=ax, axisbg="grey")
    axy.axis("off")
    ...
    for k, scen in enumerate(["RCP85", "RCP45", "RCP3PD"]):
        ...
        percentiles = da.from_pandas(contrib.quantile([0.05, 0.5, 0.95]))[
            :, plot_period]
        # plot in mm SLR
        ax.fill_between(plot_period, percentiles[
                        0.05] * 1e3, percentiles[0.95] * 1e3, color=rcpcoldict[scen], alpha=.4, lw=.5)
        ax.plot(plot_period, percentiles[0.5] * 1e3, lw=3, color=rcpcoldict[
                scen], alpha=1., label=rcpnamedict[scen])  # ,label=name)
        ...
        # axy.plot([oloc,oloc],[low,upp],lw=3,color=rcpcoldict[scen])
        low, med, upp = ipcc.ipcc_contrib_estimates[
            name][scen]  # gd.get_ipcc_range(scen,name)
